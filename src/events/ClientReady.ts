	...
	public async dispatch(): Promise<any> {
		...
		scheduleJob("0 0 * * *", async (): Promise<void> => {
			...
			const game: any = await axios.get(
				"https://api.openligadb.de/getnextmatchbyleagueteam/4608/65",
				{
			...
			if (gameData.location)
				location =
					gameData.location?.locationStadium + " - " + gameData.location?.locationCity;
			...
			if (
				...
				events?.find(
					(event) =>
						event.name ===
						"⚽ | " + gameData.team1.teamName + " vs. " + gameData.team2.teamName
				)
			)
				return;
			...
			await eventManager
				.create({
					...
					description:
						"Das nächste Spiel von Köln wird präsentiert von [E leeve lang](https://instagram.com/e.leeve.lang) ! In diesem Event siehst du das nächste Spiel unseres Effzehs! Sei gerne dabei und schreib mit uns während des Events. Klicke auf intressiert um den Spieltag nicht zu verpassen!",
					entityMetadata: {
						location: location
					},
					reason: "FC Spiel",
		...
		scheduleJob("0 0 * * *", async (): Promise<void> => {
			...
			const topThreeUsersBanner = new AttachmentBuilder(
				"../../assets/banner_message_stats.png"
			);
			let messagesEmbed;
			if (count < 250) {
				messagesEmbed = this.client.createEmbed(
					"Ein depremierender Tag für unseren Server! Heute wurden leider nur **{0} Messages** geschrieben. Dabei waren **{1} Menschen** beteiligt. Der aktivste User war <@{2}>! Danke für eure Aktivität <3",
					null,
					"normal",
					count,
					writers,
				...
				messagesEmbed = this.client.createEmbed(
					"Hey, morgen wirds besser! Heute wurden **{0} Messages** geschrieben. Dabei haben **{1} Menschen** geschrieben. Dabei war <@{2}> der aktivste! Danke für eure Aktivität <3 ",
					null,
					"normal",
					count,
					writers,
				...
				messagesEmbed = this.client.createEmbed(
					"Klasse! Heute wurden **{0} Messages** von **{1} Personen** geschrieben! Der aktivste User war <@{2}>! Danke für eure Aktivität <3",
					null,
					"normal",
					count,
					writers,
				...
				messagesEmbed = this.client.createEmbed(
					"Sehr Stark!!! Alleine heute wurden **{0} Messages** geschrieben. Dabei haben **{1} Leute** geholfen! Der aktivste User war <@{2}>! Vielen Dank für eure Aktivität <3",
					null,
					"normal",
					count,
					writers,
				...
				messagesEmbed = this.client.createEmbed(
					"Wenn selbst Hennes stolz ist, dann wisst ihr, ihr habts geschafft! Heute wurden **{0} Messages** von insgesamt **{1} Menschen** geschrieben! Der aktivste User war <@{2}> Vielen Dank für eure Aktivität <3",
					null,
					"normal",
					count,
					writers,
		...
		scheduleJob("0 0 * * 6", async (): Promise<void> => {
			...
			const guild: any = this.client.guilds.cache.get(this.client.config.support["ID"]);
			const channel: any = guild.channels.cache.get("813887099065073714");
			const selkeSamstagEmbed: any = this.client.createEmbed(
				"Es ist wieder soweit... **Es ist...**",
				null,
				"normal"
			);
			selkeSamstagEmbed.setImage(
				"https://cdn.discordapp.com/attachments/1116797977432961197/1175857152598999192/SelkeSamstag.png?ex=656cc14f&is=655a4c4f&hm=9ae60215fa91e52d215ef7e1888034698fcbb53ebf1e23118d3ac9ee74c71967&"
			);
