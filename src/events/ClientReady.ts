	...
	public async dispatch(): Promise<any> {
		...
		scheduleJob("0 0 * * *", async (): Promise<void> => {
			...
			await eventManager.create({
				...
				description: "Das nächste Spiel von Köln wird präsentiert von [E leeve lang](https://instagram.com/e.leeve.lang) ! In diesem Event siehst du das nächste Spiel unseres Effzehs! Sei gerne dabei und schreib mit uns während des Events. Klicke auf intressiert um den Spieltag nicht zu verpassen!",
				entityMetadata: {
					location: location
				},
				reason: "FC Spiel",
