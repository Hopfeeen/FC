...
export default class AvatarCommand extends BaseCommand {
    ...
    public async dispatch(interaction: any, data: any): Promise<void> {
        ...
        await this.messagesDelete(interaction.options.getUser("member"), interaction.options.getInteger("anzahl"));
    }
    ...
    private async messagesDelete(user: any, anzahl: number): Promise<any> {
        ...
        if(messagesFile.users[user.id]) messagesFile.users[user.id] = messagesFile.users[user.id] - anzahl;
        fs.writeFileSync("./assets/messages.json", JSON.stringify(messagesFile,null, 4))
        const embed: EmbedBuilder = this.client.createEmbed("{0} wurden {1} Nachrichten abgezogen.", "success", "normal", user.toString(), anzahl);
