...
export default class NevarCommand extends BaseCommand {
    ...
    private async geilerBot(): Promise<any> {
        ...
        const mostActiveUser = Object.keys(voiceFile.userTime).reduce((a, b) => voiceFile.userTime[a] > voiceFile.userTime[b] ? a : b);
        const mostActiveUserMessages = voiceFile.userTime[mostActiveUser];
        const nevarEmbed: EmbedBuilder = this.client.createEmbed("Die aktivsten Leute heute \n {0}", "text", "normal", resultArray.map(([id,count])=> this.client.emotes.arrow + ` **<@${id}> | ${Math.round(count) }** Minuten`).join("\n"));
